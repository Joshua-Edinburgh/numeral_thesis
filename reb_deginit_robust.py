...
for i in range(args.max_gen):       
    ...
    # ====================== Phase C ===================================
    pure_pairs = pair_gen(data_list, phA_rnds = args.pairs_teach, degnerate='none', sub_batch_size = 1)
    dege_pairs = pair_gen(data_list, phA_rnds = args.pairs_teach, degnerate='full', sub_batch_size = 1)
    #mixd_pairs = pair_gen(data_list, phA_rnds = args.pairs_teach, degnerate='mix', sub_batch_size = 1)
    # ====================== Phase A ===================================
    speaker = SpeakingAgent().to(DEVICE)
    spk_optimizer = OPTIMISER(speaker.parameters(), lr=LEARNING_RATE)
    acc_avg20 = 0
    ...
    while (Ia_cnt<args.Ia):
        Ia_cnt += 1
        data_deg = random.choice(dege_pairs)
        acc = train_phaseA(speaker, spk_optimizer, data_deg)
    print('Gen.%d @@PhaseA@@ Learn degenerate acc: %.4f'%(i, acc))
    
    Ia_cnt = 0   
    while (Ia_cnt<args.Ia):
        Ia_cnt += 1
        data_for_spk = random.choice(pure_pairs)
